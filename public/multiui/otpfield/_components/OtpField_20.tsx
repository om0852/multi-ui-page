...
// OTP Slot Component with Smooth Entry and Invalid Input Animation
...
export const InputOTPSlot: React.FC<InputOTPSlotProps> = ({
  ...
  return (
    <motion.input
      ...
      onKeyDown={onKeyDown}
      className={`w-14 h-14 text-center text-xl font-semibold rounded-lg border-2 transition-all duration-300 focus:outline-none focus:ring-2 ${
        invalid
          ? "border-yellow-400 bg-yellow-50 text-yellow-600 ring-yellow-500"
          : "border-blue-400 bg-blue-50 text-blue-600 ring-blue-500"
      }`}
      animate={{
        scale: value ? 1.1 : 1, // Smooth scale effect when entering text
        opacity: value ? 1 : 0.8, // Fade effect when typing
        y: value ? [0, -10, 0] : 0, // Slight bounce effect on entry
      }}
      transition={{
        duration: 0.4,
        ease: "easeInOut",
