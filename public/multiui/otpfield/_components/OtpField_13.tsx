...
export const InputOTPSlot: React.FC<InputOTPSlotProps> = ({
  ...
  return (
    <motion.input
      ...
      onKeyDown={onKeyDown}
      className={`w-14 h-14 text-center text-3xl font-bold border-2 rounded-lg transition-all duration-300 focus:outline-none ${
        invalid
      ...
      animate={{
        scale: value ? 1.1 : 1,
        opacity: value ? 1 : 0.7,
        y: invalid ? [0, -5, 5, 0] : 0, // Bounce effect on invalid input
      }}
      transition={{
        duration: 0.5,
        ease: "easeInOut",
