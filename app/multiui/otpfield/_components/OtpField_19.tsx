...
// OTP Slot Component with Fade-in and Out Animation for Entering Text and Pulse Animation for Invalid Input
...
export const InputOTPSlot: React.FC<InputOTPSlotProps> = ({
  ...
  return (
    <motion.input
      ...
      onKeyDown={onKeyDown}
      className={`w-16 h-16 text-center text-3xl font-semibold rounded-lg border-2 transition-all duration-300 focus:outline-none focus:ring-2 ${
        invalid
          ? "border-red-400 bg-red-50 text-red-600 ring-red-500"
          : "border-teal-400 bg-teal-50 text-teal-600 ring-teal-500"
      }`}
      animate={{
        opacity: value ? 1 : 0.8, // Fade effect for entering values
        scale: value ? 1.1 : 1, // Zoom effect on entering text
        y: value ? [0, -10, 0] : 0, // Bounce effect for entry
      }}
      transition={{
        duration: 0.3,
        ease: "easeInOut",
...
export const InputOTPSeparator = () => (
  <div className="text-2xl font-medium text-gray-500">|</div>
);
