...
export const PasswordInput: React.FC<PasswordInputProps> = ({
  ...
  return (
    ...
    >
      <motion.div
        className="relative bg-gradient-to-r from-blue-500 to-indigo-600 rounded-lg shadow-xl overflow-hidden"
        animate={{
          boxShadow: isFocused
        ...
        <div className="px-6 py-4">
          ...
          <div className="relative">
            <input
              ...
              className="block w-full pr-10 sm:text-sm border-none rounded-md focus:ring-2 focus:ring-indigo-300 bg-transparent outline-none placeholder-transparent"
            />
            <motion.button
              type="button"
        ...
        <AnimatePresence>
          {password && (
            <motion.div
              ...
              transition={{ duration: 0.3 }}
              className="bg-indigo-100 dark:bg-indigo-900 px-6 py-4 rounded-b-lg"
            >
      ...
      <motion.button
        className="mt-4 w-full py-3 px-4 bg-indigo-700 hover:bg-indigo-800 text-white font-medium rounded-lg text-lg transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
...
const Criteria: React.FC<{ label: string; met: boolean }> = ({ label, met }) => (
  ...
  >
    <motion.div
      className={`flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center ${met ? 'bg-green-100 dark:bg-green-800' : 'bg-red-100 dark:bg-red-800'}`}
