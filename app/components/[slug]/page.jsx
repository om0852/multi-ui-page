...
const tomorrow = lazy(() => import('react-syntax-highlighter/dist/esm/styles/prism').then(mod => ({
...
const ErrorComponent = ({ error }) => (
  <div className="flex flex-col justify-center items-center h-full min-h-[300px] p-6 text-center">
    <div className="bg-red-900/20 p-4 rounded-lg border border-red-500/30 mb-4">
      ...
      <p className="text-gray-300 text-sm">{error?.message || "The component could not be loaded. It might not exist yet or there may be an error in the component code."}</p>
    </div>
    <p className="text-gray-400 mt-4 text-sm">You can still view the code examples in the other tabs.</p>
  </div>
);
...
export default function ComponentPage() {
    ...
    const fetchComponentData = async () => {
      try {
        ...
        const variants = Array.from({ length: maxVariants }, (_, i) => ({
          id: `${i + 1}`,
          name: `Variant ${i + 1}`,
          description: `${toTitleCase(slug.replace('-', ' '))} design variation ${i + 1}`
        }));
    ...
    const importComponent = async () => {
      try {
        ...
        const Component = dynamic(
          () => import(`../../../public/multiui/${slug}/examples/Example_${variantId}`).catch(error => {
  ...
  const getComponentPath = () => {
    return `/multiui/${slug}/_components/${toTitleCase(slug)}_${selectedVariant?.id || '1'}.tsx`;
  };
  
  const getExamplePath = () => {
    return `/multiui/${slug}/examples/Example_${selectedVariant?.id || '1'}.tsx`;
  };

  // Generate code examples based on the selected component - memo these for performance
  const componentCode = React.useMemo(() => getComponentCode(), [slug, selectedVariant]);
  const exampleCode = React.useMemo(() => getExampleCode(), [slug, selectedVariant]);
  const installCode = React.useMemo(() => getInstallCode(), []);
  const usageCode = React.useMemo(() => getUsageCode(), [slug, selectedVariant]);
  ...
  return (
    <div 
      className={cn(
        ...
        {
          "bg-blue-50 border-blue-200": variant === "${selectedVariant?.id || '1'}",
...
import React from 'react';
import { ${toTitleCase(slug)} } from '../_components/${toTitleCase(slug)}_${selectedVariant?.id || '1'}';
  ...
  return (
    <div className="p-4 space-y-4">
      <${toTitleCase(slug)}
        ...
        content="This is an example of the ${toTitleCase(slug)} component with variant ${selectedVariant?.id || '1'}."
        variant="${selectedVariant?.id || '1'}"
      />
...
function MyComponent() {
  ...
  const CodeBlock = ({ code, language, style }) => (
    <Suspense fallback={<div className="p-4 bg-gray-800 text-gray-400 font-mono text-sm">Loading code...</div>}>
      <SyntaxHighlighter 
        language={language} 
        style={tomorrow}
        customStyle={{
  ...
  if (error || !component) {
    return (
      <div className="min-h-screen bg-gray-900 text-white flex flex-col justify-center items-center">
        ...
        <p className="mt-2 text-gray-300">The component you&apos;re looking for doesn&apos;t exist.</p>
        <Link 
          href="/components" 
      ...
      <section className="relative overflow-hidden py-16">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="mb-8">
            ...
            <p className="mt-2 text-lg text-gray-300">{component.description}</p>
          </div>
        </div>
      </section>
                      ...
                      >
                        ...
                        >
                          <Zap className={`h-4 w-4 mr-2 ${
                            selectedVariant?.id === variant.id ? 'text-blue-400' : 'text-gray-400'
                          }`} />
                          <div>
                            <div className="font-medium">{variant.name}</div>
                            <div className="text-xs text-gray-400 truncate">{variant.description}</div>
                          </div>
                        </button>
                      </motion.div>
                    ))}
                          ...
                          <div className="w-full">
                            <div className="bg-gray-800 border border-gray-700 rounded-lg p-6">
                              <div className="flex items-center justify-between mb-4">
                                <h3 className="text-lg font-bold text-white">{component.name} - Variant {selectedVariant?.id}</h3>
                        ...
                        <div className="p-4 bg-gray-800 border-t border-gray-700">
                          <div className="flex justify-between">
                            ...
                            >
                              ...
                              View full details for {component.name} Variant {selectedVariant?.id}
                            </Link>
                          </div>
                        </div>
                      </div>
                    ...
                    <div>
                      <div className="mb-4">
                        ...
                        <p className="text-gray-400 text-sm">
                          This is the source code for the {component.name} component, Variant {selectedVariant?.id}.
                        </p>
                      </div>
                      
                      <div className="bg-gray-900 rounded-lg overflow-hidden mb-6">
                        <div className="flex items-center justify-between bg-gray-800 px-4 py-2 border-b border-gray-700">
                          <span className="text-gray-300 text-sm font-mono">{getComponentPath()}</span>
                          <button 
                      ...
                      <div className="bg-blue-900/20 rounded-lg p-4 border border-blue-900/30">
                        ...
                        <div className="bg-gray-900 rounded overflow-hidden mb-2">
                          <CodeBlock 
                            code={installCode} 
                            language="bash" 
                            style={{
                  ...
                  {activeTab === 'example' && (
                    <div>
                      <div className="mb-4">
                        ...
                        <p className="text-gray-400 text-sm">
                          Example of how to use the {component.name} component, Variant {selectedVariant?.id} in your React application.
                        </p>
                      </div>
                      
                      <div className="bg-gray-900 rounded-lg overflow-hidden mb-6">
                        <div className="flex items-center justify-between bg-gray-800 px-4 py-2 border-b border-gray-700">
                          <span className="text-gray-300 text-sm font-mono">{getExamplePath()}</span>
                          <button 
                      ...
                      <div className="mt-6 bg-blue-900/20 rounded-lg p-4 border border-blue-900/30">
                        ...
                        <p className="text-gray-300 text-sm">
                          You can customize any component further by passing additional props or using the 
